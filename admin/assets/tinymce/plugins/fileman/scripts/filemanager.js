...
(function($) {
    ...
    var loadConfigFile = function(type) {
        ...
        var url = './connectors/php/config.php?jsmode=true';
    ...
    var getFileInfo = function(file) {
        ...
        template += '<button id="parentfolder"><i class="icon mail reply"></i> ' + lg.parentfolder + '</button>';
        if ($.inArray('select', capabilities) != -1 && ($.urlParam('CKEditor') || window.opener || window.tinyMCEPopup || $.urlParam('field_name'))) template += '<button id="select" name="select" type="button" value="Select"><i class="icon check"></i> ' + lg.select + '</button>';
        if ($.inArray('download', capabilities) != -1) template += '<button id="download" name="download" type="button" value="Download"><i class="icon download disk"></i> ' + lg.download + '</button>';
        if ($.inArray('rename', capabilities) != -1 && config.options.browseOnly != true) template += '<button id="rename" name="rename" type="button" value="Rename"><i class="icon tag"></i> ' + lg.rename + '</button>';
        if ($.inArray('move', capabilities) != -1 && config.options.browseOnly != true) template += '<button id="move" name="move" type="button" value="Move"><i class="icon move"></i> ' + lg.move + '</button>';
        if ($.inArray('delete', capabilities) != -1 && config.options.browseOnly != true) template += '<button id="delete" name="delete" type="button" value="Delete"><i class="icon trash"></i> ' + lg.del + '</button>';
        if ($.inArray('replace', capabilities) != -1 && config.options.browseOnly != true) {
            template += '<button id="replace" name="replace" type="button" value="Replace"><i class="icon exchange"></i> ' + lg.replace + '</button>';
